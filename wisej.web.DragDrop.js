...
qx.Class.define("wisej.web.DragDrop", {
	...
	properties: {
		 ...
		 */
		dropSource: { init: null, nullable: true, transform: "_transformDropSource" },
		 ...
		 */
		dropTarget: { init: null, nullable: true, transform: "_transformDropTarget" },
	...
	members: {
		...
		/**
		 * Converts a component string id to a suitable drop & drop component.
		 */
		_transformDropSource: function (value) {

			var component = this._transformComponent(value);
			if (component && component.getDragDropSource instanceof Function)
				component = component.getDragDropSource();
		 ...
		 * Converts a component string id to a suitable drop & drop component.
		 */
		_transformDropTarget: function (value) {

			var component = this._transformComponent(value);
			if (component && component.getDragDropTarget instanceof Function)
				component = component.getDragDropTarget();
		...
		*/
		__handleDrop: function (e) {
			...
			var allowFolders = qx.core.Environment.get("allow.drop.folders");

			if (allowFolders && e.dataTransfer.items) {
				this.__getFileEntries(e.dataTransfer.items, function (files) {
					this.__handleDropFiles(e, files);
				});
			}
			else if (e.dataTransfer.files) {
				this.__handleDropFiles(e, e.dataTransfer.files);
			}
		},
		__handleDropFiles: function (e, files) {
				...
				this.uploadFiles(
					...
					// callbacks
					{
						...
						completed: function (error) {
							...
							if (error)
								Wisej.onException(error);
						},

						progress: function (evt) {

							target.fireWidgetEvent("progress", { loaded: evt.loaded, total: evt.total });
		...
		__getFileEntries: function (items, callback) {
			...
			files.$$recursion = 1;
			me.__traverseItems(entries, files, callback);
		},
		__traverseItems: function (entries, files, callback) {
				...
				if (entry) {
					if (entry.isFile) {
						...
						entry.createReader().readEntries(function (children) {
							me.__traverseItems(children, files, callback);
						});
					}
				}
			}
			...
			if (files.$$recursion == 0) {
				...
				for (var i = 0; i < files.length; i++) {
					files[i].file(function (f) {
						...
						// store the relative full path.
						f.fullPath = files[ix].fullPath;
						if (f.fullPath[0] == "/")
							f.fullPath = f.fullPath.substring(1);

						files[ix] = f;

						if (++ix == files.length)
							callback.call(me, files);
					});
				}
			}
		}
