	...
	members: {
		...
		getVisibleCount: function () {
			...
			if (bounds) {
				...
				var topWidget = wisej.utils.Widget.getWidgetFromPoint(pane, 0, 1);
				var bottomWidget = wisej.utils.Widget.getWidgetFromPoint(pane, 0, bounds.height - 1);

				// to calculate the visible count we need to iterate all the visible nodes
				// and test the against the top and bottom visible nodes.
				this.__forEachNode(function (node) {
					...
					if (node == bottomWidget) {
						inRange = false;
						return false; // terminate the foreach.
					}
				...
				switch (e.getKeyIdentifier()) {
					case "Left":
						if (node.isOpen())
							...
							this.setSelection([node.getParent()]);
						return;
		...
		_onItemDrag: function (e) {
			...
			var node = this.__findNode(e.getOriginalTarget());
			if (node)
				this.fireDataEvent("itemDrag", node);
		},

		__findNode: function (target) {
			return (!target || target instanceof wisej.web.TreeNode)
				? target
				: this.__findNode(target.getLayoutParent());
		},
		...
		* Scrolls this node into view.
		...
		_applyCanExpand: function (value, old) {
			...
			if (value) {
				...
				this.setAppearance("tree-folder");
			}
			else {
				this.setOpenSymbolMode("auto");
				this.setAppearance("tree-file");
		...
		setParentNode: function (value) {
			...
			if (value != null) {
				...
				for (var node = value; node != null; node = node.getParent()) {
